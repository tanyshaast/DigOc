...
def initializeAndFind():
	...
	firstDomains=[]
	if (rank == 0):
	    for i in range (0, len(Domain)/5):
		firstDomains.append(Domain[i])
	if (rank == 1):
	    for i in range (len(Domain)/5+1,2*len(Domain)/5):
		firstDomains.append(Domain[i])
	if (rank == 2):
	    for i in range (2*len(Domain)/5+1, 3*len(Domain)/5):
		firstDomains.append(Domain[i])
	if (rank == 3):
	    for i in range (3*len(Domain)/5+1,4*len(Domain)/5):
		firstDomains.append(Domain[i])
	if (rank == 4):
	    for i in range (4*len(Domain)/5+1,len(Domain)):
		firstDomains.append(Domain[i])
        if results.compOn:
            color.log(logging.INFO, color.GREEN,
